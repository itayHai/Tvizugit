...
function Login(props) {
  ...
  allusers.push(
    <div key={0}>
      {" "}
      <UserCard key={users[0].id} icon={users[0].icon} title={users[0].name} />
    </div>
  );
  ...
  allusers.push(
    <div key={1}>
      {" "}
      <UserCard key={users[1].id} icon={users[1].icon} title={users[1].name} />
    </div>
  );
  ...
  return (
    <div>
      <div style={{ display: showLogin ? "block" : "none" }}>
        ...
        <hr color="#e6e6e6" />
        <div className={classes.Center}>
          <p>או כניסה דרך הרשתות החברתיות</p>
          <p>
            <img
